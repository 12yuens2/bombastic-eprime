...
\end{tikzpicture}
\end{minipage}
\caption{Number of solver nodes plotted against time for both initial and improved models. We can see a strong linear relationship between the time taken and number of solver nodes. This shows both are similar metrics for measuring the performance of our model.}
...
To look into more detail at how the efficiency of the models are affected and how Savile Row works at finding solutions, some of the given instances have been modified and tested against the two models for performance. This allows us to more finely test what affects the efficiency of the model and isolate specific variables to change and test.
...
A custom instance that was created and tested was an $n \times n$ cube where the avatar always started at the top left, the goal is always at the bottom right and the one single block is always on the cell directly top left of the goal. The $n \times n$ here is the size of the live cells in the problem grid, not taking into account the ring of dead cells. These problems were created to isolate two variables. First is how the size of the grid affected the time and number of nodes to find a solution. The second is to find out how Savile Row and our B0mbastic model deals with problems with multiple solutions, since all the given problem instances only have one valid solution. 
...
\end{figure}
\noindent
Cube problem sizes from $3 \times 3$ to $9 \times 9$ were created and run with the improved model. 
...
\end{tikzpicture}
\end{minipage}
\caption{Results of running all cube problems}
\end{figure}
